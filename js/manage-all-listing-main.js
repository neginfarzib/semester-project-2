...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if(listings !=null && listings.data.length > 0){
    console.log('llllising ' , listings);
  ...
  if(bidsListings !=null && bidsListings.data.length > 0){
    ...
    displayListings(myBidsThumbnail, bidsListings.data);
  }else{
  ...
  function displayListings(ListingsThumbnail, listings) {
    ...
    listings.forEach((listing) => {
      ...
      listingContent.textContent =
        listing.body?.split(/\s+/).slice(0, 50).join(' ') || '';
      ...
      createListingThumbnailBannerDiv.classList.add(
        'd-flex',
      ...
      listingBannerHrefEdit.classList.add(
        'btn',
      ...
      listingBannerHrefView.classList.add(
        'btn',
        'btn-outline-secondary',
        'btn-sm',
        'p-2'
      ...
      listingBannerHrefDelete.classList.add(
        'btn',
        'btn-outline-secondary',
        'btn-sm',
        'p-2'
